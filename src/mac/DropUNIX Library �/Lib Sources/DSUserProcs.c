...
/* sort all the options to the front of the list */
	#if defined(qWalkFolders) 
		i=1;
		j=argc-1;
		while (i<j) {
			if (argv[j][0]=='-') {
...
//		err = OpenWD (myFSSPtr->vRefNum, myFSSPtr->parID, '¶UNX', &wdRefNum);
/*		strcpy(dir,path);
		k = strrchr(dir,':');
		if (k != (char *) -1)
		{
			k++;				// k now points to the filename
			strcpy(path,k);		// strip directory from path	
			*k = '\0';			// terminate directory string
			chdir(dir);			
		}
*/			
		temp = (char *) NewPtr((len+1) * sizeof(char));
		ValidatePtr(temp);
		HLock(userDataHandle);
			fileNames = *((fileNameHandle) userDataHandle);
			fileNames->names[(fileNames->count)] = temp;
...
static void SetApplicationName(Handle userDataHandle) {
	...
	GetMyAppName(appName); // FIX: is this right?
...
//			fileNames->names[0] = (char *) NewPtr(**fileNames->names + 1); /* pstr's size in byte 0 */
			fileNames->names[0] = (char *) NewPtr((size_t) appName[0]); /* pstr's size in byte 0 */
			if (fileNames->names[0] == NULL || MemError()) {
				Panic(kMemoryError);
			}
			strcpy(fileNames->names[0], p2cstr(appName)); // BAD
		}
	HUnlock(userDataHandle);
}
...
static void AllocateAppNames(Handle *userDataHandle, unsigned long numOfFiles) {
	...
	if (fileNameHdl == NULL || (unsigned long) fileNameHdl == 1L || MemError() ) {
		Panic(kMemoryError);
	}
	...
	HLock((Handle) fileNameHdl);
		...
		/* create numOfFiles + 1 (for application name) slots to fill */
