...
void _ErrorAlert ( short stringListID, short errorStringID, short osError, Str255 pFileName, short lineNumber ) {
	#define	kAlertID	200
	#define	kOSAlertID	201
	Str255	errorString, lineNumberStr, osErrorString;

	NumToString ( lineNumber, lineNumberStr );
	GetIndString ( errorString, stringListID, errorStringID );
	
	if (osError) {
		NumToString (osError,  osErrorString);
		ParamText ( errorString, pFileName, lineNumberStr, osErrorString );
		CenterAlert ( kOSAlertID );
		(void) Alert ( kOSAlertID, NULL );
	} else {
		ParamText ( errorString, pFileName, lineNumberStr, NULL );
...
void _Panic ( short errorNumber, Str255 filename, short lineNumber) {
	...
	_ErrorAlert(kErrStringID, errorNumber, 0, filename, lineNumber);
	ExitToShell();
}

void _FailOnError(OSErr err, short errorNumber, Str255 filename, short linenumber) {

	if (err != noErr) // FIX: use err
		_ErrorAlert(kErrStringID, errorNumber, err, filename, linenumber);
}
